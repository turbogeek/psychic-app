    ...
    1: [function (require, module, exports) {
        (function (global) {
            /* MIT License
             ...
             var self = this;// For ES6
             / * global define:false * / // For ES6
             */
            /**
            ...
            (function () {
                ...
                function intersector(objects) {
                    return function intersection() {
                        ...
                        for (var jj = 0; jj <= mxj; jj++) { // loop through all array arguments
                            var array = arguments[jj],
                                mxk = array.length;
                            for (var k = 0; k < mxk; k++) { // loop through all values
                                var item = array[k];
                                if ((objects && set.has(item)) || set[item]) { // if value is possible
                                    if (jj === mxj) { // and all arrays have it (or we would not be at this point)
...
//		portions from http://phrogz.net/lazy-cartesian-product
                ...
                CXProduct.prototype.every = function (callback, pattern) {
                    ...
                    var me = this, p = [], lens = [];
                    for (var i = me.collections.length; i--;) {
                        lens[i] = me.collections[i].length;
                    }
                ...
                CXProduct.prototype.get = function (index, pattern) {
                    ...
                    var me = this, dm = [], c = [];
                    for (var f = 1, l, i = me.collections.length; i--; f *= l) {
                ...
                CXProduct.prototype.forEach2 = function (callback) {
                    var me = this, i = (typeof(me.start) === "number" ? me.start : 0),
                        max = (typeof(me.end) === "number" ? me.end : me.length());
                    while (i < max) {
                        var value = me.get(i);
                ...
                function compile(reactor, rule, boost) {
                    ...
                    rule.conditions.forEach(function (condition) {
                        var reg1 = new RegExp("/exists\(\s*(\s*{.*\s*})\s*,\s*(.*)\s*\)/", "g");
                        (condition + "").replace(reg1, // /exists\(\s*(\s*{.*\s*})\s*,\s*(.*)\s*\)/g,
                        ...
                        var reg2 = new RegExp("/forAll\(\s*(\s*{.*\s*})\s*,\s*(.*)\s*\)/", "g");
                        (condition + "").replace(reg1, // /forAll\(\s*(\s*{.*\s*})\s*,\s*(.*)\s*\)/g,
                            function (match, domainstr, conditionstr) {
                ...
                function Activation(rule, match, index, bindings, instance) {
                    ...
                    me.rule.activated++;
                    insertSorted(me.rule.reactor.agenda, me, function (a, b) {
                        return (a.rule.salience > b.rule.salience ? 1 : (a.rule.salience < b.rule.salience ? -1 : 0));
                    });
                ...
                function indexObject(index, instance) {
                    ...
                    keys.forEach(function (key) {
                        ...
                        var value = (primitive ? instance.valueOf() : instance[key]), type = typeof(value), valuekey,
                            typekey;
                        if (type === "object" && value) {
                            if (typeof(value.__rrid__) === "undefined") {
                                Object.defineProperty(value, "__rrid__", {value: UUID.generate()});
                            }
                ...
                function unIndexObject(index, instance) { // support for fixing issue #23
                    ...
                    keys.forEach(function (key) {
                        ...
                        if (!index[key][valuekey] || !index[key][valuekey][typekey]) {
                            return;
                        }
                ...
                function updateIndex(index, instance, key, oldValue) {
                    if (typeof(instance[key]) === "undefined" && !index[key]) {
                        return;
                    }
                    index[key] = (index[key] ? index[key] : {});
                    var value = instance[key], type = typeof(value), oldtype = typeof(oldValue), oldvaluekey,
                        oldtypekey, valuekey, typekey;
                    if (type === "object" && value) {
                        if (typeof(value.__rrid__) === "undefined") {
                            Object.defineProperty(value, "__rrid__", {value: UUID.generate()});
                        }
                ...
                function matchObject(index, instance, parentkeys, parentinstances) {
                    ...
                    return keys.every(function (key) {
                        ...
                        if (!index[key][valuekey]) {
                            return false;
                        }
                        if (!index[key][valuekey][typekey]) {
                            return false;
                        }
                        if (instance.__rrid__ && !index[key][valuekey][typekey][instance.__rrid__]) {
                            return false;
                        }
                    ...
                    instances.forEach(function (instance) {
                        ...
                        if (instance && typeof(instance) === "object") { // !RuleReactor.data.has(instance)
                            if (typeof(instance.__rrid__) === "undefined") {
                                Object.defineProperty(instance, "__rrid__", {value: UUID.generate()});
                            }
                                    ...
                                    if (desc.value instanceof Array || Array.isArray(desc.value)) {
                                        ...
                                        modifiers.forEach(function (fname) {
                                            ...
                                            if (typeof(f) === "function") {
                                                ...
                                                Object.defineProperty(rrget.value, fname, {
                                                    configurable: true,
                                                    writable: true,
                                                    value: newf
                                                });
                ...
                RuleReactor.forAll = function forAll(domain, test) {
                    ...
                    return test.cxproduct.every(function (row) {
                        var args = [];
                        if (!test.required || test.required.every(function (index) {
                                args.push(row[index]);
                                return typeof(row[index]) !== "undefined";
                            })) {
